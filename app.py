...
def state():
    ...
    with db() as con:
        ...
        fighters = [dict(x) for x in con.execute(
            "SELECT * FROM fighters WHERE active=1 ORDER BY rank_points DESC, record_w DESC, name ASC"
        ).fetchall()]
...
def add_idea():
    ...
    with db() as con:
        con.execute(
            "INSERT INTO ideas(user_torn_id, name, idea, created_at) VALUES(?,?,?,?)",
            (request.user["torn_id"], request.user["name"], idea[:650], now_iso()),
        )
    return jsonify({"ok": True})
...
def predict():
    ...
    with db() as con:
        ...
        try:
            con.execute("""
                INSERT INTO predictions(fight_id, user_torn_id, pick_fighter_id, points, created_at)
                VALUES(?,?,?,?,?)
            """, (fight_id, request.user["torn_id"], pick_fighter_id, points, now_iso()))
            con.execute(
                "UPDATE users SET prediction_points=prediction_points-? WHERE torn_id=?",
                (points, request.user["torn_id"]),
            )
...
@app.post("/api/admin/events/<int:event_id>/status")
@require_admin
...
def admin_result(fight_id):
    ...
    with db() as con:
        ...
        con.execute(
            "UPDATE fights SET status='done', winner_fighter_id=?, result_method=? WHERE id=?",
            (winner_fighter_id, result_method, fight_id),
        )
        con.execute(
            "UPDATE fighters SET record_w=record_w+1, rank_points=rank_points+3 WHERE id=?",
            (winner_fighter_id,),
        )
        con.execute(
            "UPDATE fighters SET record_l=record_l+1 WHERE id=?",
            (loser_id,),
        )
